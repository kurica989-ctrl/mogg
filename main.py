...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS reports (
        id SERIAL PRIMARY KEY, from_user BIGINT, target_user BIGINT,
...
def user_view_kb(user_id):
    ...
    kb.add(InlineKeyboardButton("🤝 Познакомиться", callback_data=f"askuser_{user_id}"))
    kb.add(InlineKeyboardButton("🚩 Пожаловаться", callback_data=f"report_{user_id}"))
    return kb
...
def report_reason_kb(target_id):
    kb = InlineKeyboardMarkup(row_width=1)
    reasons = ["Неприемлемое фото", "Оскорбления / харассмент", "Фейковый профиль", "Спам / реклама", "Другое"]
    for r in reasons:
        kb.add(InlineKeyboardButton(r, callback_data=f"reportreason_{target_id}_{r}"))
    return kb
...
def get_random_user(exclude_user_id, extra_exclude=None):
    ...
    conn = get_db()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    # exclude already-rated users, banned users, and profiles skipped this session
    cur.execute("""
        SELECT * FROM users
...
def save_rating(from_user, to_user, rating):
    ...
    cur.execute("""
        ...
        ON CONFLICT (from_user, to_user) DO UPDATE SET rating = EXCLUDED.rating, created_at = CURRENT_TIMESTAMP
...
def check_match(from_user, to_user, new_rating):
    ...
    if opposite and opposite['rating'] in HIGH_RATINGS:
        ...
        bot.send_message(from_user, f"❤️‍🔥 **ЛУКМЭТЧ!** ❤️‍🔥\n\nТы и @{user2['username']} лайкнули друг друга!", parse_mode='Markdown')
        bot.send_message(to_user, f"❤️‍🔥 **ЛУКМЭТЧ!** ❤️‍🔥\n\nТы и @{user1['username']} лайкнули друг друга!", parse_mode='Markdown')
    conn.close()
...
def get_user_ratings(user_id):
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute(
        "SELECT r.rating, u.username FROM ratings r JOIN users u ON r.from_user = u.id "
        "WHERE r.to_user=%s ORDER BY r.created_at DESC", (user_id,),
    )
...
def get_unread_messages(user_id):
    ...
    cur.execute(
        ...
        "WHERE m.to_user=%s AND m.read=FALSE ORDER BY m.created_at DESC", (user_id,),
    )
...
def get_matches(user_id):
    ...
    cur.execute(
        ...
        "FROM matches WHERE user1=%s OR user2=%s", (user_id, user_id, user_id),
    )
...
def start(m):
    ...
    if not user:
        ...
        bot.send_message(
            ...
            f"Сервис доступен только совершеннолетним пользователям ({MIN_AGE}+).\n\n"
            f"Сколько тебе лет?",
            parse_mode='Markdown',
        )
        return
    ...
    if user.get('registered'):
        ...
        bot.send_message(uid, "Продолжим регистрацию — просто ответь на следующий вопрос.")
...
def admin_panel(m):
    ...
    for r in reports:
        kb = InlineKeyboardMarkup(row_width=2)
        kb.add(
            InlineKeyboardButton("⛔ Забанить", callback_data=f"adminban_{r['target_user']}_{r['id']}"),
            InlineKeyboardButton("✅ Отклонить", callback_data=f"admindismiss_{r['id']}"),
        )
        bot.send_message(
            uid,
            f"🚩 Жалоба #{r['id']}\nНа: @{r['target_username']} (id {r['target_user']})\nПричина: {r['reason']}",
            reply_markup=kb,
        )
...
def handle_text(m):
    ...
    if not user.get('age') and m.text.isdigit():
        ...
        bot.send_message(uid, "📏 Твой рост? (см, число от 100 до 250)")
        return

    if user.get('age') and not user.get('height'):
        if not m.text.isdigit() or not (100 <= int(m.text) <= 250):
            bot.send_message(uid, "❌ Введи рост числом от 100 до 250 см.")
            return
        update_user(uid, height=int(m.text))
        bot.send_message(uid, "⚖️ Твой вес? (кг, число от 30 до 250)")
        return

    if user.get('height') and not user.get('weight'):
        if not m.text.isdigit() or not (30 <= int(m.text) <= 250):
            bot.send_message(uid, "❌ Введи вес числом от 30 до 250 кг.")
            return
    ...
    # --- report reason typed as free text ---
    if state.get("action") == "reporting_other":
        save_report(uid, state.get("target_id"), m.text[:300])
        bot.send_message(uid, "✅ Жалоба отправлена модераторам.", reply_markup=main_menu())
        notify_admins(f"🚩 Новая жалоба на пользователя id {state.get('target_id')}:\n{m.text[:300]}")
        clear_state(uid)
        return
    ...
    if m.text == "🎲 Оценить":
        ...
        target = get_user(state.get("target_id"))
        clear_state(uid)
        if target:
            bot.send_message(uid, "✅ Письмо отправлено! Теперь оцени анкету 👇")
...
def handle_voice(m):
    ...
    if state.get("action") == "sending_message" and state.get("msg_type") == "voice":
        ...
        clear_state(uid)
        if target:
            bot.send_message(uid, "✅ Голосовое письмо отправлено! Теперь оцени анкету 👇")
            show_rating_card(uid, target)
        else:
...
def handle_circle(m):
    ...
    if state.get("action") == "sending_message" and state.get("msg_type") == "circle":
        ...
        clear_state(uid)
        if target:
            bot.send_message(uid, "✅ Кружок отправлен! Теперь оцени анкету 👇")
            show_rating_card(uid, target)
        else:
...
def show_rating_card(uid, target):
    text = (
        f"👤 **@{target['username']}**  ·  {target['age']} лет\n"
        f"━━━━━━━━━━━━━━━\n"
        f"📏 {target['height']} см   ⚖️ {target['weight']} кг\n"
        f"🏙️ {target['city']}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"📝 _{target['bio']}_\n"
        f"━━━━━━━━━━━━━━━\n"
...
def rate_menu(uid):
    ...
    if not target:
        # ran out of fresh profiles — reset the skip list and try once more
        if skipped:
            skipped_profiles[uid] = set()
            target = get_random_user(uid)
        if not target:
            bot.send_message(uid, "😢 Больше некого оценивать прямо сейчас — загляни позже", reply_markup=main_menu())
            return
    show_rating_card(uid, target)
...
def set_rating(c):
    ...
    if rating in HIGH_RATINGS:
        ...
        bot.send_message(
            target_id,
            f"{emoji} **@{user['username']}** оценил тебя на **{rating}**!\n\n👁️ Посмотреть анкету?",
            reply_markup=user_view_kb(uid), parse_mode='Markdown',
        )
    # auto-advance to the next profile
    rate_menu(uid)
...
def show_top(m):
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT u.username, u.gender, COUNT(r.id) as count
        FROM users u LEFT JOIN ratings r ON r.to_user = u.id
...
def handle_askuser(c):
    ...
    bot.send_message(
        target_id,
        f"🤝 **@{user['username']}** хочет с тобой познакомиться и просит твой юзернейм!",
        reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("✅ Дать юз", callback_data=f"giveuser_{uid}")),
        parse_mode='Markdown',
    )
...
def handle_report_reason(c):
    ...
    bot.send_message(uid, "✅ Жалоба отправлена модераторам.", reply_markup=main_menu())
    notify_admins(f"🚩 Новая жалоба на пользователя id {target_id}:\nПричина: {reason}")
...
def handle_admin_ban(c):
    ...
    bot.edit_message_text(f"⛔ Пользователь id {target_id} забанен. Жалоба закрыта.", c.message.chat.id, c.message.message_id)
    try:

...
def user_view_kb(user_id):
    ...
    kb.add(InlineKeyboardButton("📞 Запросить контакт", callback_data=f"askuser_{user_id}"))
...
def get_random_user(exclude_user_id):
    conn = get_db()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    # exclude already-rated users so people don't get the same profile twice
    cur.execute("""
        SELECT * FROM users
...
def rate_menu(m):
    ...
    if not target:
        bot.send_message(uid, "😢 Больше некого оценивать прямо сейчас — загляни позже")
        return
    text = (
        f"👤 **@{target['username']}** ({target['name']})\n\n"
        f"📅 {target['age']} лет\n📏 {target['height']} см\n⚖️ {target['weight']} кг\n"
        f"🏙️ {target['city']}\n📝 _{target['bio']}_\n\n💖 **Оцени внешность:**"
    )
...
def handle_askuser(c):
    ...
    bot.send_message(
        target_id,
        f"📞 **@{user['username']}** запросил(-а) твой контакт!",
        reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("✅ Дать контакт", callback_data=f"giveuser_{uid}")),
        parse_mode='Markdown',
    )

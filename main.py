...
def rated_profile_kb(target_id):
    ...
    kb.row(
        InlineKeyboardButton(
            "⭐ Оценить в ответ",
            callback_data=f"replyrate_{target_id}"
        ),
        InlineKeyboardButton(
            "⏭️ Пропустить",
            callback_data=f"replyskip_{target_id}"
        )
    )
...
def user_view_kb(user_id):
    ...
    kb.add(
        InlineKeyboardButton(
            "👀 Показать анкету",
            callback_data=f"showrated_{user_id}"
        )
    )
...
def check_match(
    ...
    try:
        bot.send_message(
            ...
            f"Второй пользователь: @{username2}",
            parse_mode="Markdown"
        )
    except Exception:
        pass
    ...
    try:
        bot.send_message(
            ...
            "❤️‍🔥 *Лукмэтч!* ❤️‍🔥\n\n"
            f"Вы понравились друг другу!\n"
            f"Второй пользователь: @{username1}",
            parse_mode="Markdown"
        )
    except Exception:
        pass
...
def notify_about_rating(
    ...
    text = (
        ...
        f"Оценка: *{rating}*\n\n"
        f"👀 Показать анкету этого человека?"
    )
    ...
    try:
        bot.send_message(
            to_user,
            text,
            reply_markup=user_view_kb(from_user),
            parse_mode="Markdown"
        )
    except Exception:
        pass
...
def update_daily_bonus(user_id):
    ...
    cur.execute(
        "SELECT daily_bonus_date, daily_bonus_count, rating FROM users WHERE id=%s",
        (user_id,)
    )
    row = cur.fetchone()
    ...
    if row and row[0] == today:
        ...
        if count == 5:  # срабатывает ровно один раз при достижении 5
            current_rating = float(row[2] or 0)
            update_user(
                user_id,
                rating=round(current_rating + 0.5, 2),
                weekly_top=1
            )
            try:
                bot.send_message(
                    ...
                    "Рейтинг +0.5 и ты в топе недели!",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
...
def handle_text(m):
    ...
    if user.get("age") and not user.get("height"):
        if (
            ...
            bot.send_message(
                uid,
                "❌ Рост нужно указать числом от 100 до 250 см."
            )
    ...
    if user.get("height") and not user.get("weight"):
        if (
            ...
            bot.send_message(
                uid,
                "❌ Вес нужно указать числом от 30 до 250 кг."
            )
    ...
    if user.get("weight") and not user.get("city"):
        ...
        bot.send_message(
            ...
            "Можно пропустить — просто напиши *пропустить*",
            parse_mode="Markdown"
        )
    ...
    if user.get("city") and user.get("bio") is None:
        # bio ещё не задан (None) — принимаем текст или пропуск
        ...
        bot.send_message(
            ...
            "(лучше селфи или портрет в хорошем качестве)",
            parse_mode="Markdown"
        )
...
def handle_photo(m):
    ...
    if (
        user
        and user.get("city")
        and user.get("bio") is not None   # bio уже пройден (даже если пустой)
...
@bot.callback_query_handler(
    func=lambda c: c.data and c.data.startswith("gender_")
)
def set_gender(c):
    """Выбор пола — всегда отвечаем на callback сразу, чтобы кнопка не крутилась."""
    uid = c.from_user.id

    # 1. Сразу гасим "часики" на кнопке — это самое важное
    try:
    ...
    try:
        ...
        # Убираем старое сообщение с кнопками
        try:
        ...
        # Если что-то упало — всё равно сообщаем пользователю
        try:
            bot.send_message(
                uid,
...
@bot.callback_query_handler(
    func=lambda c: c.data.startswith("showrated_")
)
def show_rated(c):
    uid = c.from_user.id
...
@bot.callback_query_handler(
    func=lambda c: c.data.startswith("replyskip_")
)
...
@bot.callback_query_handler(
    func=lambda c: c.data.startswith("replyrate_")
)
def handle_reply_rate(c):
    ...
    bot.send_message(
        uid,
        "⭐ **Оцени его/её в ответ:**",
        reply_markup=reply_rating_kb(
            target["gender"],
            target_id
        ),
        parse_mode="Markdown"
...
def show_ratings(m):
    ...
    if not ratings:
        bot.send_message(
            uid,
            "📊 Пока никто тебя не оценил 🥺\n"
            "Оценивай других — и оценки скоро появятся!",
            reply_markup=main_menu()
        )
...
def show_messages(m):
    ...
    if not messages:
        bot.send_message(
            ...
            "Когда кто-то напишет — сразу увидишь 💌",
            reply_markup=main_menu()
        )
...
def show_matches(m):
    ...
    if not matches:
        bot.send_message(
            uid,
            "❤️‍🔥 Пока мэтчей нет\n"
            "Оценивай людей высоко — и взаимность обязательно появится 💫",
            reply_markup=main_menu()
        )
...
def show_top(m):
    ...
    if not top_users:
        bot.send_message(
            uid,
            "🏆 Пока топ пуст — будь первым, кто начнёт оценивать!",
            reply_markup=main_menu()
        )

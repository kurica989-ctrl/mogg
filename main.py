...
# age gate matters. If you deliberately want a different value, change
# it here explicitly rather than lowering it by accident.
MIN_AGE = 18
...
def init_db():
    ...
    # Migration: add any columns that didn't exist in earlier deployments.
    # CREATE TABLE IF NOT EXISTS does NOT alter an already-existing table,
    # so this is required whenever new columns are introduced.
    new_columns = [
...
def create_user(user_id, username, name):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO users (id, username, name) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
...
# =========================================================
# TOP — anonymized: name + rank, no username
# =========================================================
...
def show_top(m):
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT u.name, u.gender, u.rank, COUNT(r.id) AS count
        FROM users u
        LEFT JOIN ratings r ON r.to_user = u.id
        WHERE u.registered=TRUE AND u.is_banned=FALSE
        GROUP BY u.id
    ...
    for i, u in enumerate(top_users, 1):
        ...
        text += f"{place} {emoji} {u['name']} {rank_emoji} — {u['count']} 🌟\n"
    bot.send_message(uid, text, parse_mode="Markdown", reply_markup=main_menu())
